...
public class CoroutineQueue
{
    ...
    public CoroutineQueue Move(GameObject obj, Vector3 newPos, float time, bool isLocal=false)
    {
        waitQueue.Add(MoveObj(obj, newPos, time, isLocal));
        return this;
    }
    ...
    IEnumerator MoveObj(GameObject obj, Vector3 newPos, float time, bool isLocal)
    {
        Vector3 prePos = isLocal ? obj.transform.localPosition : obj.transform.position;
        float t = 0;
        while (t < 1)
        {
            ...
            if (isLocal)
            {
                obj.transform.localPosition = Vector3.Lerp(prePos, newPos, t);
            }
            else {
