...
public class WeaponScript : MonoBehaviour {
	...
	protected virtual void Update () {
		...
		ani.SetFloat("curSpeed",curSpeed);
		ani.SetBool ("jumping", jumping);
		ani.SetBool ("reloading", reloading);

		//If there a character controller, check if we are grounded
		if (co != null)
			ani.SetBool ("grounded", co.isGrounded);
		else
	...
	protected virtual void Shoot () {
		...
		if (aiming)
			ani.CrossFade ("AimShoot", 0.02f, 0, 0);
		else
			ani.CrossFade ("Shoot", 0.02f, 0, 0);
	...
	// 	{
	// 		if ( hit.collider.CompareTag("Terrorist") )
	// 		{
	// 			Debug.Log("SHOOT");
	// 		}
	// 	}
